% TAIPS_01C - Simulator pentru determinarea automata a gradului optim p
% Si salvarea zgomotului colorat aferent fiecarui canal (FAZA II)
clear; clc; close all;
...
% =========================================================================
% FAZA II: SALVAREA ZGOMOTULUI COLORAT
% =========================================================================

nume_fisier_zgomot = sprintf('V%02d.mat', n);

% Verificam daca fisierul exista deja sau daca exista mai multe canale
if nr_canale > 1 && exist(nume_fisier_zgomot, 'file')

    load(nume_fisier_zgomot, 'V');
    
    % Ne asiguram ca dimensiunile coincid (cazul in care K se schimba)
    % Daca exista mai multe canale, actualizam doar canalul ales
    if size(V.y, 1) ~= length(v_p)
         warning('Lungimea zgomotului curent difera de cea salvata anterior. Se suprascrie tot fisierul.');
         zgomot = zeros(length(v_p), nr_canale);
         zgomot(:, ch) = v_p;
         V = iddata(zgomot, [], 1);
    else
         V.y(:, ch) = v_p;
    ...
    % Daca avem un singur canal sau fisierul nu exista, il (re)initializam
    if nr_canale > 1
        ...
        % Cazul cu un singur canal (nu este nevoie de o matrice 'zgomot')
        V = iddata(v_p, [], 1);
    end
    
    V.Name = ['Zgomot colorat ' nume_fisier];
    V.TimeUnit = Y.TimeUnit;
    V.Notes = sprintf('Generat din %s cu p=%d si K=%d. Contine reziduul v pe orizontul de masura.', nume_fisier, p_0, K);
end

% Salvarea pe disc
save(nume_fisier_zgomot, 'V');